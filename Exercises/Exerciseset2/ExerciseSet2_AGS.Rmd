...
**Your Answers here:**
<br>
1. We need 6 factors (we can see it by sorting the last column of the PCA table in ascending order).  
<br>
2. 0.93 on each the 5 yr T-Notes US and the 10yr T-Notes, 0.86 on Treasury Bonds, etc.  
<br>
3. We can interpret them as portfolios of futures that explain most of the `r ncol(futures_data)` futures' variance.  
<br>
4.a. Pre-crisis data  
<br>
```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
precrisis_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" , ]
...
```
<br>
```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data = data.frame(Variance_Explained_Table)
m1<-gvisTable(show_data,options=list(width=1920, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
...
```
<br>
5. In precrisis years, most of the variance in the world economy is explained by bonds, currencies, indexes, and oil to some extent. During the crisis, the variance is driven by bonds, currencies, commodities, and indexes.  
<br> 
<hr>
...
**Your Answers here:**  
1. Lines 283-287 construct for each day (row) and each future (column) the sum of flags for the various moving average windows contained in the vector MAfreq.  
Lines 289-293 that each row's module is equal to 1.  
Line 297 shifts the data vertically so that we use the signals from 2 days ago.  
Line 299 sums up the normalized signals row-wise.  
<br>
2.  
```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,180,by=10)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
...
names(trading_strategy) <- rownames(futures_data)
```
With moving average windows of `r MAfreq`, here are the performance results:
<br>
<br>
